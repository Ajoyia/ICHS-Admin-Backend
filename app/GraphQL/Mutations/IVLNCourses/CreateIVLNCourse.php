...
final class CreateIVLNCourse
{
    ...
    public function __invoke($_, array $args)
    {
        ...
        if(isset($args['model_type'])){
            $course->model_type=$args['model_type'];
        }
        if(isset($args['model_id'])){
            $course->model_id=$args['model_id'];
        }
        ...
        if(isset($args['speciality_id'])){
            $course->speciality_id=$args['speciality_id'];
        }
        ...
        $chkP=DB::table('ivln_courses')->where('slug',$course->slug)->get();
        if(count($chkP)){
            $course->slug=$course->slug.'-'.IvlnCourse::max('id')+1;
        }
